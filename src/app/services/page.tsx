    ...
    <div className="container mx-auto px-4 py-16 md:py-20 lg:py-24">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
        {services.map((service, index) => (
          <Card
            key={service.id}
            className="card-base group flex flex-col justify-between text-center overflow-hidden animate-subtle-slide-up bg-card border border-border/30 hover:border-primary/50 transition-colors duration-300" // Removed card-hover and 3D styles, adjusted styles
            style={{ animationDelay: `${index * 0.1 + 0.2}s` }}
          >
            <CardHeader className="items-center pt-8 pb-4">
              <div className="mb-5 flex items-center justify-center h-16 w-16 rounded-full bg-gradient-to-br from-primary/10 to-accent/10 text-primary transition-all duration-300 group-hover:scale-110 group-hover:bg-primary/20 group-hover:text-primary"> {/* Adjusted icon background */}
